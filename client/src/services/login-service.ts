...
export class LoginServices {
  ...
  public login = async (user: UserModel): Promise<void> => {
    const response = await this.http.post(this.url + "/login", user);
  ...
  public register = async (user: UserModel): Promise<void> => {
    const response = await this.http.post(this.url, user);
    this.handleSuccessResponse(response);
  }
  ...
  public handleSuccessResponse = async (response): Promise<UserModel> => {
  ...
  public handleRouting = (navigation, user: UserModel): void => {

    user.isAdmin === 1 ?
      navigation.push(`/admin`)
      : navigation.push(`/user/${user.uuid}`);
  };
  // // end of function
